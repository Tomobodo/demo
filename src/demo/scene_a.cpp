...
void scene_a(float time, const unsigned int frame, const Rect &src_rect,
  ...
  const auto draw_ball_x =
      abs((ball_incr + h_range) % (h_range * 2) - h_range) + BSIZE;
  ...
  const int y_offset =
      static_cast<int>(fast_sin(time) * BACKGROUND_CIRCLE_RADIUS);
  const int x_offset =
      static_cast<int>(fast_cos(time) * BACKGROUND_CIRCLE_RADIUS);
