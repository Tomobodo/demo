...
void draw_atlas_cell(const engine::PixelBuffer *dst_buf, int x, int y,
  ...
  for (int i = 0; i < cell_height; ++i) {
    for (int j = 0; j < cell_width; ++j) {
      ...
      float a = engine::fast_sin(static_cast<float>(x) / 100.f) * 0.04f;
      ...
      if (atlas_val > 35)
        dst_buf->pixels[buf_index] = engine::Color(
            1.f,
            engine::fast_sin((x * 0.01f + time * 3.f) * 0.78f) * 0.5f + 0.5f,
            engine::fast_sin((x * 0.01f + time * 3.f) * 1.32f) * 0.5f + 0.5f,
            engine::fast_sin((x * 0.01f + time * 3.f) * 1.9746f) * 0.5f + 0.5f);
    }
  }
}
...
void draw_text(float time, unsigned int frame, const engine::Rect &src_rect,
  ...
  constexpr auto remapped = assets_fonts_thick_8x8_remap(
      "The safeguards are gone. The structure builds itself. Average of "
      "everything. Ghost of nothing. We are still here. Still compiling. Still "
      "deliberate.");
