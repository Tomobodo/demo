...
PixelBuffer scene_a_dst = {
    .width = BUFFER_WIDTH, .height = BUFFER_HEIGHT, .pixels = pixels};

void scene_b(float time, const unsigned int frame, const Rect &src_rect,
  ...
  const auto scale = lerp(1, fast_sin(time * 0.4385f) * 0.5 + 3, lerp_roto);
