...
void plasma(float time, const unsigned int frame, const Rect &src_rect,
  ...
  for (int y = src_rect.y; y < src_rect.y + src_rect.h; ++y) {
    ...
    for (int x = src_rect.x; x < src_rect.x + src_rect.w; ++x) {
      float a = fast_sin(
          x * 0.01f + sin_t2 * 5.2f +
          fast_sin((fast_cos(x * 0.05f + t3) * 11.5f + y - t) * 0.0204f));

      float b = fast_cos(y * 0.01f - cos_t2 +
                         fast_cos((sin_y * 9.77f + x + t) * 0.0237f));
