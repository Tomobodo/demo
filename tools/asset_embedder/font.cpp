...
void FontEmbedder::parse_font(const fs::path &path, pugi::xml_node &root,
	...
	for (const auto &char_node : chars_nodes.children()) {
		...
		if (!id_attr || !x_attr || !y_attr || !advance_attr || !page_attr ||
	...
	for (const auto &page_path : pages_path) {
		...
		unsigned char *pixels =
				stbi_load(page_path.string().c_str(), &width, &height, &n, 0);
	...
	engine::Font font = {.glyphs = final_font_glyphs.data(),
											 .width =
													 static_cast<unsigned char>(glyphs.begin()->width),
	...
	generated_code << "};" << std::endl
								 << std::endl
								 << "constexpr engine::Font " << font_variable << " = {"
								 << std::endl
	...
	generated_code << "template<unsigned int N>" << std::endl
								 << "struct " << font_variable << "_remapped { char data[N]; };"
								 << std::endl;

	// string conversion
	generated_code << std::endl
								 << "template<unsigned int N>" << std::endl
								 << "constexpr auto " << font_variable
								 << "_remap(const char (&str)[N]) {" << std::endl
	...
	for (int i = 0; i < 128; ++i) {
		for (int j = 0; j < glyphs.size(); ++j) {
			...
			if (glyph.id == i)
				generated_code << "\tconversion_table[" << i << "] = " << j + 1 << ";"
											 << std::endl;
		}
	}

	generated_code << std::endl
								 << "\t" << font_variable << "_remapped<N> result{};"
								 << std::endl;

	generated_code << "\tint i = 0;" << std::endl
								 << "\tfor(unsigned int i = 0; i < N; ++i) {" << std::endl
								 << "\t\tresult.data[i] = conversion_table[str[i]];"
								 << std::endl
								 << "\t}" << std::endl
...
void FontEmbedder::on_command(std::string_view &command, const void *data) {
	if (command == "register_string") {
		std::string_view str(static_cast<const char *>(data));
		for (size_t i = 0; i < str.length(); ++i) {
			m_required_char.insert(str.at(i));
		}
	}
}

void FontEmbedder::embed_file(const std::filesystem::path &file_path,
															std::set<std::string_view> &includes,
															std::stringstream &generated_code) {
	pugi::xml_document doc;
	pugi::xml_parse_result result = doc.load_file(file_path.c_str());
