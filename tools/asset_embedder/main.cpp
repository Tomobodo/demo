...
int main(int argc, char **argv) {
	if (argc < 4) {
		std::println(std::cerr, "Usage : PROJECT_ROOT_PATH "
														"ASSETS_DIRECTORY_PATH OUTPUT_HEADER_PATH");
		return 1;
	}
	...
	embedders.push_back(std::make_unique<StringEmbedder>(embedders));
	embedders.push_back(std::make_unique<FontEmbedder>(embedders));
	embedders.push_back(std::make_unique<PaletteEmbedder>(embedders));
